...
sim::security::ContextHandle LoadViaGateway(sim::security::SecurityHardware* hardware, const sim::isa::AsmProgram& program,
                                            std::uint32_t user_id, std::uint32_t key_id,
                                            std::uint64_t entry_offset = 0,
                                            const std::vector<std::uint8_t>* code_memory_override = nullptr) {
  sim::security::Gateway gateway(*hardware);
  sim::security::SecureIrBuilderConfig config;
  config.program_name = "executor_test_program";
  config.user_id = user_id;
  config.key_id = key_id;
  config.window_id = 1;
  config.entry_offset = entry_offset;

  sim::security::SecureIrPackage package = sim::security::SecureIrBuilder::Build(program, config);
  if (code_memory_override != nullptr) {
    package.code_memory = *code_memory_override;
  }
  return gateway.Load(std::move(package)).handle;
}

sim::core::ExecResult RunAllowAll(const sim::isa::AsmProgram& program, std::uint64_t saved_pc,
...
RunOutcome RunWithEwc(const std::string& text, std::uint64_t base_va, const sim::security::EwcTable& ewc,
  ...
  sim::security::SecurityHardware hardware;
  const sim::security::ContextHandle handle = LoadViaGateway(&hardware, program, 1, 0, entry_offset, code_memory);
  hardware.GetEwcTable() = ewc;
  hardware.GetAuditCollector().Clear();
  if (code_memory == nullptr) {
    std::uint32_t key_id = 0;
    const sim::security::EwcQueryResult query_result = hardware.GetEwcTable().Query(base_va + entry_offset, handle);
    ...
    region->code_memory = local_code_memory;
  }
  hardware.SetActiveHandle(handle);
  ...
  options.hardware = &hardware;
  RunOutcome outcome;
  outcome.result = sim::core::ExecuteProgram(options);
  outcome.audit_events = hardware.GetAuditCollector().GetEvents();
  outcome.context_handle = handle;
  return outcome;
}
...
sim::core::ExecResult RunWithHardware(const std::string& text, std::uint64_t base_va,
  ...
  const sim::isa::AsmProgram program = sim::isa::AssembleText(text, base_va);
  sim::security::SecurityHardware hardware;
  const sim::security::ContextHandle handle = LoadViaGateway(&hardware, program, user_id, key_id);
  hardware.SetActiveHandle(handle);
...
SIM_TEST(Execute_NonZeroEntryOffset_StartsFromSavedPc) {
  const std::string src = R"(
  LI x1, 99
  LI x2, 7
  HALT
)";
  const sim::isa::AsmProgram program = sim::isa::AssembleText(src, 0x4100);

  const auto result = RunAllowAll(program, 0x4104);

  SIM_EXPECT_EQ(result.trap.reason, sim::core::TrapReason::HALT);
  SIM_EXPECT_EQ(result.state.regs[1], 0u);
  SIM_EXPECT_EQ(result.state.regs[2], 7u);
  SIM_EXPECT_EQ(result.trap.pc, 0x4108u);
}

SIM_TEST(Execute_MaliciousOsCannotChooseFirstEntryPc) {
  const std::string src = R"(
  LI x5, 123
  HALT
)";
  const sim::isa::AsmProgram program = sim::isa::AssembleText(src, 0x4200);

  const auto result = RunAllowAll(program, 0x4204);

  SIM_EXPECT_EQ(result.trap.reason, sim::core::TrapReason::HALT);
  SIM_EXPECT_EQ(result.state.regs[5], 0u);
  SIM_EXPECT_EQ(result.trap.pc, 0x4204u);
}
...
SIM_TEST(Execute_EwcAllows_ProgramRunsToHalt) {
  ...
  w.code_policy_id = 1;
  ewc.SetWindows(1, std::vector<sim::security::ExecWindow>{w});

  const RunOutcome outcome = RunWithEwc(src, base, ewc);
...
SIM_TEST(Executor_HardwarePath_NormalExecution) {
  ...
  const auto result = RunWithHardware(src, 0x9800, 8, 41);
  SIM_EXPECT_EQ(result.trap.reason, sim::core::TrapReason::HALT);
  SIM_EXPECT_EQ(result.state.regs[3], 16u);
  SIM_EXPECT_EQ(result.context_trace.size(), static_cast<std::size_t>(1));
  SIM_EXPECT_TRUE(Contains(result.context_trace[0], "context_handle=1"));
}
...
SIM_TEST(Executor_HardwarePath_NoActiveHandle_Error) {
  ...
  sim::core::ExecuteOptions options;
  options.hardware = &hardware;
  const auto result = sim::core::ExecuteProgram(options);

  SIM_EXPECT_EQ(result.trap.reason, sim::core::TrapReason::INVALID_PC);
  SIM_EXPECT_EQ(result.trap.pc, 0u);
...
SIM_TEST(Execute_EwcDenies_AtEntry_TrapsEwcIllegalPc) {
  ...
  SIM_EXPECT_EQ(outcome.audit_events[0].context_handle, outcome.context_handle);
  ...
  SIM_EXPECT_TRUE(Contains(outcome.result.trap.msg, "context_handle=1"));
}
...
SIM_TEST(Execute_PseudoDecrypt_CorrectKey_RunsToHalt) {
  ...
  const RunOutcome outcome = RunWithEwc(src, base, ewc, &code_memory);
