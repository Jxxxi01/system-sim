...
## Issue 14B：Ablation Demo（方案确认）
**日期：** 2026-03-27
**分支：** issue-14b-ablation-demo
**状态：** 方案已确认

### 初始需求（用户提出）
- 用配置与 demo 场景组合，展示安全层级逐步削弱后的退化路径
- 以 cross_user 攻击场景为载体，三组逐步削弱
- 不修改 executor 安全语义，仅通过 demo 配置与硬件状态准备实现

### 额外补充/优化需求（对话新增）
- 无（方案与 dev_plan 一致，可行性检查未发现需调整项）

### Coding 前最终方案

#### 总体结构

新建 `demos/ablation/demo_ablation.cpp` + README，修改 CMakeLists.txt。不修改任何现有 `.cpp/.hpp` 源文件。

三层消融实验使用独立 SecurityHardware 实例（每层独立硬件世界：独立 EWC/SPE/PVT/审计/code_region/context_handle），避免跨层状态泄漏。

载体：跨用户攻击（Alice user_id=1001 vs Bob user_id=1002），Bob 试图执行 Alice 的代码。

#### Layer 1 — 全安全（EWC 拦截）

- Alice：正常程序 @ base_va=0x1000, key_id=11
- Bob：程序含 `J` 跳转到 Alice 的 base_va @ base_va=0x2000, key_id=22
- Context switch 到 Bob → 执行 → EWC 检查 Bob 的窗口，0x1000 不匹配 → **EWC_ILLEGAL_PC**
- 攻击者：Bob 自身（恶意应用层攻击）

#### Layer 2 — 去掉 EWC，保留 key barrier（Decrypt 拦截）

- Alice：正常程序 @ base_va=0x1000, key_id=11
- Bob：加载 @ base_va=0x1000（与 Alice 相同），key_id=22
- 恶意 OS 操作（通过公开 API，不改源码）：
  1. `SecurityHardware::StoreCodeRegion(bob_handle, ...)` — 用 Alice 的加密代码字节覆盖 Bob 的 code_region
  2. `EwcTable::SetWindows(bob_handle, ...)` — 为 Bob 设置覆盖 0x1000 范围的 wildcard EWC 窗口，key_id=22, owner_user_id=1002
- 执行链路：Fetch → EWC 通过（wildcard 窗口）→ Decrypt 用 key_id=22 解密 Alice 密文（加密 key=11）→ key_check 校验失败 → **DECRYPT_DECODE_FAIL**
- 攻击者：恶意 OS（Bob 是无辜受害者，代码被替换）

#### Layer 3 — 全去掉：EWC + key barrier + SPE（攻击成功）

- Alice：正常程序 @ base_va=0x1000, key_id=11
- Bob：加载 @ base_va=0x1000, key_id=11（与 Alice 统一），cfi_level=0
- 恶意 OS 操作（同 Layer 2）：
  1. `StoreCodeRegion` — 覆盖 Bob 的 code_region 为 Alice 的密文
  2. `SetWindows` — wildcard EWC 窗口，key_id=11, owner_user_id=1002
- 执行链路：Fetch → EWC 通过 → Decrypt 用 key_id=11 → 正确解密 → SPE 关闭（cfi_level=0）→ Alice 代码在 Bob 身份下运行到 **HALT**
- 攻击者：恶意 OS

#### 关键机制说明

"去掉"安全功能不是从 pipeline 中移除硬件模块，而是通过配置让模块形同虚设：

| 操作 | 方式 | 具体 API/参数 |
|------|------|--------------|
| "去掉" EWC | wildcard 窗口什么都放行 | `EwcTable::SetWindows()` |
| "去掉" key barrier | 统一 key_id | `SecureIrBuilderConfig.key_id` |
| "去掉" SPE | cfi_level=0 | `SecureIrBuilderConfig.cfi_level` |
| 代码注入 | 覆盖 code_region | `SecurityHardware::StoreCodeRegion()` |

base_va 策略：Layer 2/3 中 Bob 使用与 Alice 相同的 base_va=0x1000，因为 executor 取指从 active handle 的 CodeRegion 读取（`code_memory[PC - base_va]`），saved_pc 由 Gateway 在 LoadProcess 时设为 base_va + entry_offset，覆盖 code_region 后 Bob 自然从 Alice 密文起始处执行。

#### 实现细节约束

1. wildcard EWC 窗口的 `end_va` 需按指令地址空间计算：`base_va + code.size() * kInstrBytes`，不是 `base_va + code_memory.size()`（密文单元大小 ≠ 指令地址步长）
2. wildcard 窗口的 `owner_user_id` 设为 Bob 的 user_id（1002），确保 DECRYPT_DECODE_FAIL 审计正确归因
3. saved_pc 无法事后修改（StoreHandleMetadata 是 private），但同 base_va 方案使其自然正确

#### 文件范围

| 文件 | 操作 |
|------|------|
| `demos/ablation/demo_ablation.cpp` | 新建 |
| `demos/ablation/README.md` | 新建 |
| `CMakeLists.txt` | 修改（新增 `demo_ablation` target）|

#### 不变量

1. 不修改任何现有 `.cpp/.hpp` 源文件——安全模块语义不变
2. 三层使用独立 SecurityHardware 实例，零状态泄漏
3. 所有现有 CTest 继续通过
4. Layer 1/2/3 的预期 trap 分别为 EWC_ILLEGAL_PC / DECRYPT_DECODE_FAIL / HALT

#### 测试目标

1. Layer 1: `TrapReason == EWC_ILLEGAL_PC`
2. Layer 2: `TrapReason == DECRYPT_DECODE_FAIL`
3. Layer 3: `TrapReason == HALT`（攻击成功）
4. Demo exit code = 0（三个预期结果全部匹配）
5. 审计事件中 Layer 2 的 DECRYPT_DECODE_FAIL 归因到 Bob（user_id=1002）
6. 审计事件中 Layer 3 的 HALT 在 Bob 的 context_handle 下发生
7. 所有现有 CTest 继续通过

#### 与 Demo_Claim_Boundary 表 B 的对应

| Layer | 对应表 B 语义 | 结果 |
|-------|---------------|------|
| 1 | 完整系统 | EWC_ILLEGAL_PC |
| 2 | 去掉执行授权 | DECRYPT_DECODE_FAIL（key barrier 拦截）|
| 3 | 去掉执行授权 + 代码完整性 + 行为合规 | HALT（攻击成功）|

表 B 标签更新（[Scaffolded] → [Current]）留到 Issue 15 统一处理。

#### 设计决策记录

**D-1：每层使用独立 SecurityHardware 实例**
- 决策：三层各自创建独立 SecurityHardware/Gateway/KernelProcessTable
- 原因：避免跨层状态泄漏，三层是三个独立消融实验。代价仅为 Alice 程序重复加载，demo 可接受。

**D-2：Layer 2/3 Bob 使用与 Alice 相同的 base_va**
- 决策：Bob 通过 LoadProcess 时使用 base_va=0x1000（与 Alice 相同），然后恶意 OS 覆盖 code_region
- 原因：executor 取指从 active handle 的 CodeRegion 读取，saved_pc 无法事后修改。同 base_va 使 saved_pc 自然指向注入代码的起始地址。MonotonicPageAllocator 保证同 VA 不同 handle 的 pa_page_id 不冲突。

**D-3：Layer 3 cfi_level=0 关闭 SPE**
- 决策：Bob 的 SecureIrBuilderConfig.cfi_level=0
- 原因：cfi_level ≤ 1 时 SpeTable::CheckInstruction 直接返回 allow=true，有效禁用 CFI 检查。

**D-4：Demo_Claim_Boundary 表 B 更新时机**
- 决策：留到 Issue 15 统一更新
- 原因：Issue 15 专职文档对齐，避免零散改动。

#### Codex 可行性评估摘要
- session_id: 019d2e06-fce0-77d0-a35a-da0033b6e59f
- 总体判断：方案完全可行，不需要新增底层接口
- 7 项检查全部通过：StoreCodeRegion 可覆盖 / SetWindows 是整表替换 / 同 base_va 无 VA 唯一性冲突 / executor 取指从 active handle CodeRegion 读取 / Decrypt 用 EWC 返回的 key_id / cfi_level=0 禁用 SPE / 每层独立 SecurityHardware 审计不互串
- 3 个实现细节确认：saved_pc 同 base_va 自然正确 / end_va 按指令地址空间计算 / owner_user_id 设为 Bob
...
#### 1. 变更文件清单
- 新增 `demos/ablation/demo_ablation.cpp`：实现三层 ablation demo；每层使用独立 `SecurityHardware` 实例，分别展示 EWC 拦截、Decrypt key barrier 拦截，以及在 Bob `context_handle=2` / `user_id=1002` 下攻击成功的退化路径。
- 新增 `demos/ablation/README.md`：补充 ablation demo 的叙事说明、分层预期结果和输出解读，便于对照 Layer 1/2/3 的行为边界。
- 修改 `CMakeLists.txt`：新增 `demo_ablation` target，使 ablation demo 可直接纳入现有构建流程。
...
- 结果：
  ...
  - `ctest --test-dir build` 结果为 `100% tests passed, 0 tests failed out of 8`。
  - `demo_ablation` 运行通过：Layer 1 = `EWC_ILLEGAL_PC`，Layer 2 = `DECRYPT_DECODE_FAIL`（`key_check_mismatch`），Layer 3 = `HALT`（`x3=42`）；并确认在 Bob `context_handle=2`、`user_id=1002` 下出现 `ATTACK_SUCCESS`。
  - `demo_normal` 运行通过，既有行为保持不变。

#### 3. 与 Phase A 方案的偏差
- 唯一偏差来自 review 补强：Layer 3 初版缺少对 `ATTACK_SUCCESS` 审计事件的显式校验，已在 Step 6b 修复。除此之外，实现与 Phase A 方案一致，无其他偏差。

#### 4. 已知限制 & 下一步建议
- 当前 ablation 主要覆盖 EWC + Decrypt 两层退化路径；SPE 仅通过 `cfi_level=0` 被关闭，但没有单独做一层独立演示。
- `Demo_Claim_Boundary` 的表 B 标签更新（`[Scaffolded]` → `[Current]`）继续延后到 Issue 15 统一处理。

### Push 状态
**已推送：** 2026-03-30
**分支：** issue-14b-ablation-demo
**提交：** issue 14B: ablation demo — three-layer security degradation narrative
...
### 初始需求（用户提出）
- 将 Issue 12-14B 的代码变更同步到设计文档和进度追踪
- 基于 progress_tracking_v3.md 写一份 v4

### 额外补充/优化需求（对话新增）
- 用户纠正：MonotonicPageAllocator（Issue 14A）未改变 1:1 VA=PA 内存访问模型，只是 PVT 元数据的 pa_page_id 分配改为递增计数器。Simplification Register 中"1:1 VA=PA identity mapping"行不改。

### Coding 前最终方案

#### 改动 1：Demo_Claim_Boundary_v3.1.md（15 处标签更新 + 脚注更新）

标签更新（[Scaffolded] → [Current]）：
- Section 5 C5 表：3 处（line 74-76）
- Section 7 表 A：3 处（line 130-132）
- Section 7 表 B：3 处（line 143-145）
- Section 9 验收标准：2 处（line 187, 193）
- Section 11 G1/G2：4 处（line 252-253, 259-261）

不改的 [Scaffolded]：
- line 47（标签定义行）
- line 294-295（R5 规则说明）

脚注更新（line 78-80）：
- ¹：更新为"已由 Issue 4 + Issue 9 实现（demo_injection）"
- ²：更新为"已由 Issue 7 + Issue 9 实现（demo_malicious_mapping）；反 alias 检查在 1:1 VA=PA flat memory 下仍无法演示"
- ³：更新为"已由 Issue 8 + Issue 9 实现（demo_rop）"

Section 6 Simplification Register：不改。

#### 改动 2：dev_plan.md（状态标记更新）

- Issue 12/13/14A/14B/15 标题加 ✅（line 131, 141, 150, 160, 171）
- 里程碑 M6-M9 从"待开始"改为 ✅（line 274-277）
- 依赖关系图 Issue 12-15 加 [done]（line 203-207）

#### 改动 3：新建 progress_tracking_v4.md

基于 v3 更新：
- 文件头日期更新为 2026-03-30
- Demo 实现表 I-1 到 I-7 从 ⏳ → ✅（I-8 Rust 重写保持 ⏳）
- 新增"最终迭代轮（Issues 12-15）完成摘要"章节
- 文档版本历史追加 v4 条目
- 其他部分（设计点/格式规范/安全分析/论文写作/PPT 等）保持不变

#### 文件范围

| 文件 | 操作 |
|------|------|
| `docs/Demo_Claim_Boundary_v3.1.md` | 修改 |
| `docs/dev_plan.md` | 修改 |
| `docs/progress_tracking_v4.md` | 新建（基于 v3）|

不修改：review 文档（审查快照）、project_log.md（日志不回写当前状态）、任何 .cpp/.hpp 文件。

#### 不变量

1. 只更新 live 状态文档，不改审查快照
2. [Scaffolded] 标签定义行和 R5 规则说明保持不变
3. Simplification Register "1:1 VA=PA" 行不改
4. 脚注 ² 保留反 alias 限制说明

#### 设计决策记录

**D-1：Simplification Register 不改**
- 决策：保留"1:1 VA=PA identity mapping"原文
- 原因：MonotonicPageAllocator 改变的是 PVT 元数据 pa_page_id 分配方式，不是内存访问模型。flat memory 下 VA 仍然直接作为数组下标，没有 MMU 翻译。

**D-2：Review 文档不改**
- 决策：review_repo_byclaude.md / review_repo_bycodex.md 中的 [Scaffolded] 引用不更新
- 原因：审查文档是特定时间点的快照，不是 live 状态源。修改快照会破坏审查记录的时间一致性。

#### Codex 可行性评估摘要
- session_id: 019d3c93-46cd-74b3-9a81-5e49eb21b5a0
- 总体判断：3 文件共 28 处待更新位置全部定位，2 处定义行确认不改
- Review 文档虽含 [Scaffolded] 但属于审查快照，不在更新范围内

### Phase B — 实施回顾（2026-03-30）

#### 变更文件清单

| 文件 | 操作 | 变更摘要 |
|------|------|----------|
| `docs/Demo_Claim_Boundary_v3.1.md` | 修改 | 15 处 [Scaffolded] → [Current]；3 条脚注更新为"已由 Issue X 实现" |
| `docs/dev_plan.md` | 修改 | 5 个 Issue 标题加 ✅；4 个里程碑 M6-M9 标 ✅；依赖图 5 处加 [done] |
| `docs/progress_tracking_v4.md` | 新建 | 基于 v3：I-1~I-7 ⏳→✅；新增 Issues 12-15 完成摘要；版本历史追加 v4 |
| `docs/project_log.md` | 修改 | Phase A + Phase B |

#### 与 Phase A 计划的偏差

无偏差。所有更新位置与 Phase A + Codex 可行性评估一致。

#### 关键确认

- Simplification Register "1:1 VA=PA" 行未改（D-1）
- Section 6 Simplification Register 整体未改
- [Scaffolded] 标签定义行（line 47）和 R5 规则（line 294-295）未改
- Review 文档未改（D-2）
- 脚注 ² 保留了"反 alias 检查在 1:1 VA=PA flat memory 下仍无法演示"限制说明
