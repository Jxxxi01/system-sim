...
| 项目 | 内容 |
|------|------|
| 一句话目标 | 引入最小内核模拟器，实现 `secure_context_switch(handle)` 切换 active context，并实现 cross_user demo |
| 对应模块 | I-6 扩展（kernel emulator）|
| 前置依赖 | Issue 5（Gateway 提供 `context_handle` 分配机制） |
| 预期产出 | 新增：`include/kernel/kernel_emu.hpp`, `src/kernel/kernel_emu.cpp`, `tests/test_kernel.cpp`, `demos/cross_user/demo_cross_user.cpp`；修改：`include/core/executor.hpp`, `CMakeLists.txt` |
| 核心验收标准 | 1. `secure_context_switch` 正确切换 active context；2. 切换后 EWC query 使用新 `context_handle`；3. cross_user demo：Alice 正常执行 → 切到 Bob → Alice 代码触发 `EWC_ILLEGAL_PC` |
      ...
      -> Issue 3 (EWC)    [done]
        -> Issue 4 (Pseudo Decrypt)
          -> Issue 5 (Gateway + SecureIR)
            -> Issue 6 (Kernel + Context Switch + cross_user demo)
            |   -> Issue 7 (PVT)
          ...
          -> Issue 6 [also depends on Issue 5]

Issue 9 -> Issue 10 (Audit normalization) [optional]
```

核心安全故事最小闭环：**Issue 4 + 5 + 6**。完成后具备：伪加密/解密 + Gateway 自动配置 EWC + 上下文切换 + cross_user demo。
...
| M3：跨用户安全闭环 | 4 + 5 + 6 | 上下文切换 + cross_user demo |
