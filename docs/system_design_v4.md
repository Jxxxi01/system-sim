...
>
> v4 更新说明：
> - DP-9：SecureIR明文/加密分区明确（布局元数据明文，内容和策略加密）
> - DP-10：OS地址空间可见性明确（OS知道布局形状，不知道入口点和内容）
> - DP-11：安全上下文与普通上下文划分修正（PC在安全上下文，GPR/SP在普通上下文）
> - DP-12：SP由OS设置的风险分析（风险可接受，PVT权限检查保护代码完整性）
> - DP-13：GATEWAY_LOAD流程补充（含saved_PC初始化）
> - DP-14：首次启动流程明确（与恢复执行统一机制）
> - DP-15：PIE要求明确（代码必须位置无关）
> - Demo阶段简化说明（不支持argc/argv）
>
> v3 更新说明（保留）：
...
│                                                                              │
│  [OS 可见]                                                                   │
│    base_va               OS 自己选择并提供给 GATEWAY_LOAD                   │
│    各段 offset/size      SecureIR 明文部分                                  │
│    各段 VA 范围          OS 可计算：segment_va = base_va + offset           │
│    栈/堆约束             SecureIR 明文部分（约束，非具体位置）              │
│                                                                              │
│  [OS 不可见]                                                                 │
│    entry_offset          SecureIR 加密部分                                  │
│    entry_va              OS 无法计算（= base_va + entry_offset）            │
│    代码/数据内容         加密保护                                           │
│    CFI 策略              加密保护                                           │
│    bounds 策略           加密保护                                           │
│                                                                              │
│  [设计说明]                                                                  │
│    OS 知道程序的"地址空间形状"（各段范围），但不知道：                       │
│      - 具体从哪条指令开始执行（entry_va）                                   │
│      - 代码/数据的内容                                                      │
│      - 安全策略细节                                                         │
│                                                                              │
│    这个设计是功能与安全的平衡：                                             │
│      - OS 需要布局信息来完成加载功能（分配PA、配置页表）                   │
│      - 布局信息泄露的安全影响低（只知道"形状"不知道"内容"）                │
│      - 真正敏感的信息（代码、数据、策略）都是加密的                         │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
### [已确定] GATEWAY_LOAD 完整处理流程（v4 新增）

```
OS 调用：
  context_handle = GATEWAY_LOAD(secureir_addr, secureir_len, base_va)

Gateway 内部处理：
═══════════════════════════════════════════════════

[阶段 1: 读取与解密]
  1.1  从 secureir_addr 读取 SecureIR
  1.2  读取明文部分：user_pubkey, encrypted_K, segments[], constraints
  1.3  用 hw_sk 解密 encrypted_K → 获得密钥 K
  1.4  用 K 解密 [K加密区域] → 获得 entry_offset, CFI策略, bounds策略等

[阶段 2: 验证]
  2.1  验证 signature（用 user_pubkey 验证整个 SecureIR 完整性）
  2.2  计算 user_id = hash(user_pubkey)
  2.3  验证 entry_offset 落在某个 code_segment 范围内
  2.4  验证资源请求不超配额

[阶段 3: 计算地址]
  3.1  对每个 segment：segment_va = base_va + offset
  3.2  计算 entry_va = base_va + entry_offset（OS 不可见）

[阶段 4: 配置硬件]
  4.1  配置 EWC 执行窗：
         添加执行窗 [segment_va, segment_va + size)
         设置 owner = user_id, key_id, type, code_policy_id
  4.2  配置 SPE 策略表：
         写入 cfi_level, call_targets[], jmp_targets[]
         写入 bounds_policy[]
  4.3  将 K 存入密钥存储（按 user_id 索引）

[阶段 5: 初始化安全上下文]
  5.1  分配 context_handle（硬件管理的不透明句柄）
  5.2  初始化安全上下文槽：
         saved_PC = entry_va        ← 关键：首次执行的 PC
         user_id = 计算得到的 user_id
         ewc_config = 当前 EWC 配置快照
         spe_config = 当前 SPE 策略快照
         shadow_stack = 空
         shadow_stack_ptr = 0
         previous_user_id = INVALID

[阶段 6: 返回]
  返回 context_handle 给 OS
  （不返回 entry_va，OS 不知道程序入口点）
```
...
## 5.1 [已确定] SecureIR完整格式定义（v4 更新）

### [已确定] SecureIR 明文/加密分区（v4 新增）
...
│                                                                              │
│  设计原则：                                                                  │
│    - 明文区域：OS 完成加载功能所必需的信息，签名保护完整性                  │
│    - 加密区域：代码/数据内容 + 安全策略 + 入口点，OS 不可见                 │
│    - 密钥 K 用 hw_pk 加密，OS 只能看到 encrypted_K 密文                     │
│                                                                              │
│  [明文区域] OS 可读取，签名覆盖保护完整性                                   │
│  ├── user_pubkey                    // 用户公钥                             │
│  ├── signature                      // 对整个 SecureIR 的签名               │
│  ├── encrypted_K                    // Enc(hw_pk, K)，密钥密文              │
│  ├── code_segments[]:                                                       │
│  │     offset                       // 相对 base_va 的偏移                  │
│  │     size                         // 段大小                               │
...
│  │     size                         // 段大小                               │
│  │     permissions                  // RW | RO                              │
│  ├── stack_constraints:             // 栈约束（非指定具体位置）             │
│  │     min_size, max_size                                                   │
│  ├── heap_constraints:              // 堆约束（非指定具体位置）             │
│  │     max_size                                                             │
│  └── trusted_libs[]                 // 信任声明                             │
│                                                                              │
│  [K加密区域] Enc(K, ...)，OS 只能看到密文                                   │
│  ├── code_segments[].content        // 加密的代码内容                       │
│  ├── code_segments[].mac            // 认证标签                             │
│  ├── data_segments[].content        // 加密的数据内容                       │
│  ├── data_segments[].mac            // 认证标签                             │
│  ├── entry_offset                   // 入口点偏移（相对 base_va）           │
│  ├── cfi_level                      // 1 | 2 | 3                            │
│  ├── call_targets[]                 // 层级3 CFI目标                        │
│  ├── jmp_targets[]                  // 层级3 CFI目标                        │
│  └── bounds_policy[]                // SPE bounds策略                       │
│                                                                              │
│  [设计说明]                                                                  │
│    布局元数据（offset/size）明文：OS 需要用于内存分配和页表配置             │
│    内容和策略加密：真正的机密，OS 不需要知道                                │
│    entry_offset 加密：OS 不需要知道程序从哪里开始执行                       │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
[密钥交换]（明文，但内容是密文）
  encrypted_K                      // Enc(hw_pk, K)，用 hw_pk 加密的用户密钥

[代码段描述]（布局明文，内容加密）
  code_segments[]:
    offset                         // 明文：相对 base_va 的偏移
    size                           // 明文：段大小
    type                           // 明文：USER_CODE | TRUSTED_LIB_REF
    permissions                    // 明文：RX（代码段）
    encrypted_content              // 加密：代码内容（库引用时为空）
    mac                            // 加密：认证标签
...
[数据段描述]（布局明文，内容加密）
  data_segments[]:
    offset                         // 明文：相对 base_va 的偏移
    size                           // 明文：段大小
    permissions                    // 明文：RW | RO
    encrypted_content              // 加密：初始化数据
    mac                            // 加密：认证标签
...
[SPE策略声明]（加密）
  cfi_level                        // 加密：1 | 2 | 3
  call_targets[]                   // 加密：层级3合法CALL目标列表
  jmp_targets[]                    // 加密：层级3合法JMP目标列表
  bounds_policy[]:                 // 加密：来源1静态声明
    segment_ref                    // 关联的代码段
    accessible_ranges[]            // 该代码段可访问的数据范围
  additional_policies[]            // [待补充] 后续加入更多安全策略

[入口点]（加密，v4 新增）
  entry_offset                     // 加密：程序入口点相对 base_va 的偏移

[资源约束]（明文）
  max_heap_size                    // 最大堆大小（约束，非指定）
  max_stack_size                   // 最大栈大小（约束，非指定）
...
│                                                                              │
│  [要求]                                                                      │
│    用户代码必须是 Position Independent Executable (PIE)                     │
│                                                                              │
│  [理由]                                                                      │
│    base_va 由 OS 选择，用户代码不能假设固定加载地址                         │
│    代码内所有地址引用必须是 PC-relative                                     │
│                                                                              │
│  [实现]                                                                      │
│    编译时：使用 -fPIE -pie 或等效选项                                       │
│    代码中：使用相对寻址访问数据、调用函数                                   │
│                                                                              │
│  [示例]                                                                      │
│    ; 访问数据                                                                │
│    lea rax, [rip + data_offset]   ; PC-relative，不依赖绝对地址             │
│                                                                              │
│    ; 调用函数                                                                │
│    call func                       ; 相对调用                               │
│                                                                              │
│  [与 SecureIR 的关系]                                                        │
│    SecureIR 中的 offset 是相对于 base_va 的偏移                             │
│    entry_offset 也是相对于 base_va 的偏移                                   │
│    OS 选择 base_va 后，所有段地址 = base_va + offset                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

位置无关编译 + 地址无关加密（PIE/PIC）
...
│ 安全上下文（片上SRAM，OS不可访问）                                           │
│                                                                              │
│ saved_PC:          下次执行的 PC（首次启动 = entry_va）（v4新增）           │
│ user_id:           该上下文的用户身份（v4新增）                             │
│ ewc_config[]:      EWC执行窗配置快照                                        │
│ spe_config[]:      SPE策略配置快照                                          │
│ shadow_stack[]:    影子栈内容（如果启用）                                   │
│ shadow_stack_ptr:  影子栈指针                                               │
│ previous_user_id:  syscall发起者的user_id                                   │
│ context_handle:    当前上下文句柄                                           │
│                                                                              │
├─────────────────────────────────────────────────────────────────────────────┤
│ 普通上下文（内核保存，OS可见）                                               │
│                                                                              │
...
│ PC 在安全上下文中：                                                          │
│   - OS 无法读取/设置运行时 PC                                               │
│   - entry_va 由 Gateway 初始化到 saved_PC，OS 不可见                        │
│   - 首次启动与恢复执行使用统一机制                                          │
│                                                                              │
│ SP 在普通上下文中：                                                          │
│   - OS 可以设置 SP（风险可接受，见下文分析）                                │
│   - 首次启动时 OS 设置 SP 指向分配的栈空间                                  │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### [已确定] SP 由 OS 设置的风险分析（v4 新增）
...
│                                                                              │
│  [机制]                                                                      │
│    首次启动：OS 分配栈空间，设置 SP 指向栈顶                                │
│    恢复执行：从普通上下文恢复 SP                                            │
│                                                                              │
│  [攻击场景分析]                                                              │
│                                                                              │
│    攻击1: OS 设置 SP 指向用户代码段                                         │
│      → 用户 push 触发写操作                                                 │
│      → PVT 检查：permissions = RX，写操作不允许                             │
│      → 结果：安全异常，攻击失败                                             │
│                                                                              │
│    攻击2: OS 设置 SP 指向用户只读数据段                                     │
│      → PVT 检查：permissions = RO，写操作不允许                             │
│      → 结果：安全异常，攻击失败                                             │
│                                                                              │
│    攻击3: OS 设置 SP 指向用户可写数据段（非栈区域）                         │
│      → PVT 检查：owner 匹配，permissions = RW                               │
│      → 结果：允许，但用户数据被栈操作覆盖                                   │
│      → 后果：程序行为异常（精细化 DoS）                                     │
│                                                                              │
│    攻击4: OS 设置 SP 指向其他用户页面                                       │
│      → PVT 检查：owner 不匹配                                               │
│      → 结果：安全异常，攻击失败                                             │
│                                                                              │
│    攻击5: OS 设置 SP 指向无效地址                                           │
...
│                                                                              │
│  [结论]                                                                      │
│    OS 设置 SP 的风险：可能导致用户可写数据被意外覆盖                        │
│    但不会：破坏代码完整性（代码段不可写）                                   │
│            泄露数据给 OS                                                    │
│            跨用户攻击（PVT owner 检查）                                     │
│    风险等级：可接受（等价于精细化 DoS，不超出 OS 已有能力）                 │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  [OS 操作]                                                                   │
│    1. GATEWAY_LOAD(secureir_addr, len, base_va) → 获得 context_handle       │
│    2. 从 SecureIR 明文部分读取 segments 布局                                │
│    3. 为各段分配 PA，配置页表（VA → PA）                                   │
│    4. 对每个代码/数据页：SECURE_PAGE_LOAD 注册到 PVT                        │
│    5. 分配栈空间，SECURE_PAGE_LOAD 注册栈页                                 │
│    6. 设置普通上下文：SP = 栈顶，其他 GPR = 0（或约定值）                   │
│    7. SECURE_CONTEXT_SWITCH(context_handle)                                 │
│    8. SYSRET                                                                │
│                                                                              │
│  [硬件行为 - SECURE_CONTEXT_SWITCH]                                         │
│    1. 保存当前安全上下文（如果有）                                          │
│    2. 查找 context_handle 对应的安全上下文槽                                │
│    3. 加载目标 EWC 配置                                                     │
│    4. 加载目标 SPE 配置                                                     │
│    5. 设置 current_context = context_handle                                 │
│                                                                              │
│  [硬件行为 - SYSRET]                                                        │
│    1. 从安全上下文读取 saved_PC（首次启动 = entry_va）                      │
│    2. 从普通上下文读取 SP、GPR                                              │
│    3. 清除 previous_user_id = INVALID                                       │
│    4. 切换到用户态                                                          │
│    5. PC = saved_PC，开始执行                                               │
│                                                                              │
│  [设计说明]                                                                  │
│    首次启动与恢复执行使用统一机制：                                         │
│      - 首次启动：saved_PC = entry_va（GATEWAY_LOAD 初始化）                │
│      - Syscall 返回：saved_PC = 被打断的 PC（SYSCALL 时保存）              │
│      - 中断返回：saved_PC = 被打断的 PC（中断时保存）                      │
│    统一语义：SYSRET = "从安全上下文恢复 PC，从普通上下文恢复 GPR"           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 5.1 | SecureIR完整格式 | **v4更新** | 明文/加密分区明确 + entry_offset |
| 5.2 | PIE要求 | **v4新增** | 代码必须位置无关 |
| 6.4 | 页表保护 | **已确定** | B-2 + must_map |
| 6.5 | 统一安全页面协议 | **已确定** | 三条专用指令 |
| 7.1 | user_id传递策略 | **已确定** | 并行方案 |
| 7.2 | 共享库处理 | **已确定** | 选项B（per-user拷贝） |
| 7.3 | 安全上下文划分 | **v4更新** | PC在安全上下文，GPR/SP在普通上下文 |
| 7.3 | SP由OS设置风险分析 | **v4新增** | 风险可接受，PVT权限检查保护 |
| 7.3 | 首次启动流程 | **v4新增** | 与恢复执行统一机制 |
...
| 附录F | Demo阶段简化 | **v4新增** | 不支持argc/argv，栈由OS设置 |
...
│                                                                              │
│  [首次启动参数]                                                              │
│    Demo 不支持 argc/argv 传递                                               │
│    用户程序不接收命令行参数                                                 │
│    如需"参数"：编译时硬编码，或通过其他机制获取                            │
│                                                                              │
│  [栈管理]                                                                    │
│    OS 分配栈空间并设置 SP                                                   │
│    用户代码信任 SP 初始值                                                   │
...
│  [堆管理]                                                                    │
│    可采用静态分配或简化的动态分配                                           │
│    完整的动态 bounds 支持为 future work（见 E.2）                           │
│                                                                              │
│  [共享库]                                                                    │
│    Demo 可简化为不使用共享库，或使用预加载的单一 libc                       │
│    完整的库预注册模型为 future work                                         │
│                                                                              │
│  [CFI 层级]                                                                  │
│    Demo 可默认使用层级 1（仅 EWC 保护）                                     │
│    层级 2/3 的 CFI 策略验证为可选功能                                       │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
